...
incumbency_table <- fb_ads |>
  group_by(incumbency) |>
  summarize(candidate_count = n_distinct(cand_id))
knitr::kable(incumbency_table)
```

There are 2,510 challenger candidates, 2,022 incumbent candidates, and 2,482 open seat candidates. All together there are 7,014 candidates assigned in the data set.
...
```{r}
us_house_senate_races <- fb_ads |>
  filter(office %in% c("US House", "US Senate"))
party_incumbent_promote <- us_house_senate_races |>
...
knitr::kable(
  ...
  digits = 3,
  col.names = c("Party", "Incumbency", "Avg. Party Promote")
)
```

The type of candidate that sponsored the most promoting ads on average was a democratic incumbent at 0.854%.
...
plot_q3 <- fb_ads |>
  ...
  labs(x = "Ad Impressions (Millions)",
       y= "# of Candidates (Hundreds)")
plot_q3
```

The histogram is very "bottom"/left heavy, as many of the candidates fall within the 0-10 million ad impression range. Therefore, the histogram has very large and dominant bars in that range, and very short and harder to view bars when you look at the 10-30 million impressions range, meaning there are significantly fewer candidates who have achieved that many. So, most of the house candidates did not have more than 10 million impressions, but rather most of them had between 0-10 million impressions instead.
...
fb_top_plot <- ggplot(top_house_candidates, mapping = aes(x = impressions_millions, 
                                                          y = cand_name_last))+
  geom_bar(stat = "identity", aes(fill = party)) +
  labs(title = "Top 15 House Candidates by FB Ad Impressions (2017-2018)",
